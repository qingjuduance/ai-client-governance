...
- coord session、lock 或队列记录不能代替 Git live state；开始修改、恢复任务和最终收口时，
  必须用 `git worktree list` 或 `worktree-task status --write-state` 复核 worktree
  真实存在、分支正确且未被其它会话清理。
...
- 新增 runtime 节点必须同时说明触发条件、去重键、失败策略、验证证据和性能边界；
  不能让所有任务无差别慢跑同一检查。
...
- 规则/脚本/文档链路变更后，最终记录必须覆盖 runtime 节点是否可见、门禁是否执行、
