#!/usr/bin/env python3
"""Check an embedded ai-rules Git repository.
...
from ai_rules.common.paths import AI_RULES_STATE_PATH
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check embedded ai-rules Git sync state.")
    parser.add_argument("--target-project-path", default=".", help="Target project path. Default: current directory.")
    parser.add_argument("--embedded-repo-path", help="Embedded ai-rules repo path.")
    parser.add_argument("--config-path", help="ai-rules config path.")
...
def check_sync(args: argparse.Namespace) -> SyncReport:
    ...
    config_path = resolve_path(project, args.config_path, ".codex/ai-rules-config.json")
    config = read_json(config_path) or {}
    embedded_default = str(config.get("embeddedRepoPath") or ".codex/ai-rules")
    embedded_repo = resolve_path(project, args.embedded_repo_path, embedded_default)
    state_path = project / AI_RULES_STATE_PATH
    ...
    if not embedded_repo.exists():
        add_warning(
            report,
            f"Embedded ai-rules repo is missing at {repo_label}. Embed it before rule work.",
            "Embed the configured ai-rules repository before rule work.",
        )
    ...
    if inside.exit_code != 0 or inside.text != "true":
        add_warning(report, f"{repo_label} exists but is not a Git work tree.", "Review the embedded ai-rules path.")
    ...
    if report.remote_present and report.fetch_due:
        ...
        add_warning(
            ...
            "Configure an ai-rules remote or compare manually.",
        )
    ...
    if report.dirty:
        add_warning(
            ...
            "Review and intentionally commit, stash, or discard local ai-rules changes.",
        )
    ...
    if not report.branch:
        add_warning(
            report,
            f"{repo_label} is in detached HEAD state; record the intended ai-rules version explicitly.",
            "Record or switch to the intended ai-rules branch.",
        )
    else:
            ...
            if counts.exit_code == 0:
                ...
                if report.ahead > 0 and report.behind > 0:
                    add_warning(
                        ...
                        "Resolve ai-rules divergence manually.",
                    )
                elif report.ahead > 0:
                    add_warning(
                        report,
                        f"{repo_label} is ahead of {upstream_name} by {report.ahead} commit(s). Push from .codex/ai-rules when approved.",
                        "Push from .codex/ai-rules after approval.",
                    )
                elif report.behind > 0:
                    add_warning(
                        report,
                        f"{repo_label} is behind {upstream_name} by {report.behind} commit(s). Run git pull --ff-only inside .codex/ai-rules.",
                        "Run git pull --ff-only inside .codex/ai-rules after review.",
                    )
                else:
...
def render_text(report: SyncReport) -> str:
    lines = [f"AI rules sync check: {report.status}"]
    ...
    if report.warnings:
        lines.append("Warnings repeat every session until the embedded ai-rules repository is synchronized.")
    return "\n".join(lines)
