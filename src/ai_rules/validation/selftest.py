"""Black-box self-tests for ai-rules enforcement behavior."""
...
from ai_rules.common.paths import PYTHON_PYCACHE_DIR, TMP_DIR, ai_rules_entrypoint
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run ai-rules black-box self-tests.")
...
| 命令 | 工作目录 | 结果 | 摘要 |
|---|---|---|---|
| `ai_rules.py task-gate` | selftest | 由 selftest 验证 | 缺 status 应失败，补齐 status 应通过。 |
"""
...
| 命令 | 工作目录 | 结果 | 摘要 |
|---|---|---|---|
| `{root}\\scripts\\ai_rules.py task-gate --root {root}` | selftest | 通过 | 这里故意保留绝对路径命令证据，必须依赖 task tracking 上下文通过 validate-doc。 |
""",
    )
    tracking_body += f"""
## 已处理文件
...
"""
    ...
    passed = (
        result.exit_code == 0
        and "ai_rules.py validate-doc" in output
...
def main() -> int:
    ...
    run_dir = root / TMP_DIR / "ai-rules-selftest" / timestamp
