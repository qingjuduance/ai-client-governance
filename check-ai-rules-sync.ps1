...
$pythonEntry = Join-Path $ScriptDir "scripts\ai_rules.py"
if (-not (Test-Path -LiteralPath $pythonEntry)) {
    Write-Warning "Missing ai-rules CLI entry at scripts/ai_rules.py"
    if ($FailOnWarning) {
        exit 1
    }
    exit 0
...
if (-not $pythonCommand) {
    Write-Warning "Python is not available; cannot run scripts/ai_rules.py sync-check"
    if ($FailOnWarning) {
        exit 1
    }
    exit 0
